...
class HeroDetailInteractor: HeroDetailBusinessLogic {
    ...
    func getSimilarHero(primaryAttribute: String) {
        let similarHero = HeroHelper.instance.getSimilarHero(heroAttribute: HeroHelper.HeroPrimaryAttribute(rawValue: primaryAttribute) ?? .agi)
        presenter?.presentSimilarHero(hero: similarHero)
    }
}
